...
export default function EditMethodStatementPage({ params }: Props) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await getMethodStatement(resolvedParams.id);
        setForm({
          ...
          holdPoint: data.holdPoint,
          safety: data.safety || "",
          evidenceRequirement: (data.evidenceRequirement || []).join(", "),
          reworkProcedure: data.reworkProcedure || "",
          responsibleRoles: (data.responsibleRoles || []).join(", "),
          lessonLearned: data.lessonLearned || "",
        });
      } catch (err: any) {
        console.error("[EditMethodStatement] Load error:", err);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        await updateMethodStatement(resolvedParams.id, data);
        toast("Method statement berhasil diperbarui", "success");
        router.push("/admin/workforce/method-statements");
      } catch (err: any) {
        toast(err.message || "Gagal memperbarui method statement", "error");
      }
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Edit Method Statement</h1>
          <p className="text-sm text-slate-400">{form.methodCode} - {form.workItem}</p>
        </div>
      </div>
