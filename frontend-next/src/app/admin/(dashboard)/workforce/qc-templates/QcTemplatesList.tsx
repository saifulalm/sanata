...
interface QcTemplatesListProps {
  initialData: QcTemplate[];
  initialMeta: { page: number; pageSize: number; total: number; totalPages: number };
}

export function QcTemplatesList({ initialData, initialMeta }: QcTemplatesListProps) {
  ...
  const [deleteDialog, setDeleteDialog] = useState<QcTemplate | null>(null);

  const fetchData = async (page = 1) => {
    startTransition(async () => {
      try {
        const result = await getQcTemplates({
          page,
          wbsStage: wbsFilter || undefined,
        });
        setData(result.data as QcTemplate[]);
        setMeta(result.meta);
      } catch {
  ...
  const handleDelete = async () => {
    ...
    startTransition(async () => {
      try {
        await deleteQcTemplate(deleteDialog.id);
  ...
  const getItemCount = (items: QcTemplate["items"]) => {
    ...
    <div className="space-y-6">
      ...
      {data.length === 0 ? (
        <EmptyState
          ...
          title="Belum ada QC template"
          description="Tambahkan template checklist QC per WBS stage."
          action={
            <Link
              href="/admin/workforce/qc-templates/new"
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500">Nama Template</th>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500">Deskripsi</th>
                    ...
                    <tr key={item.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Link
                            href={`/admin/workforce/qc-templates/${item.id}/edit`}
      ...
      <Dialog
        ...
        description={`Hapus "${deleteDialog?.name}"?`}
        footer={
